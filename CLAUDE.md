...
See `homelab/PLAN.md` for the phased build-out plan (authoritative for current phase/status). Current position: **Phase 5 (Secrets track) CLOSED 2026-06-26** (Vaultwarden live — VM 118, Ubuntu 24.04 + Docker, Tailscale-only, replicated + PBS-imaged, restore drill PASS; tiered secrets model ADR-018; SSH key-only-root hardening; 2FA recovery anchors). The **HA-expansion sub-track was split out to a standalone project** (HACS done; Node-RED/ESPHome/HA→Grafana/wall-tablet deferred). **Phase 6 next** (Media — Jellyfin + qBittorrent/Gluetun). Terraform import deferred to cluster scale (ADR-008). Order: 1 VLANs ✓ → 2 Tailscale + Technitium ✓ → 3 Foundation + observability ✓ → 4 Multi-node cluster + HA ✓ (apophis + carter; oneill stays **standalone**; ZFS replication; **no HA manager/fencing** — manual failover) → 5 Secrets ✓ (Vaultwarden; HA-expansion split out) → 6 Jellyfin + media. Cross-cutting: backups + patching.
