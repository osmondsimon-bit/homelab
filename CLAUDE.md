...
See `homelab/PLAN.md` for the phased build-out plan (authoritative for current phase/status). Current position: **Phase 4 CLOSED 2026-06-25** (Multi-node + HA: 2-node cluster `homelab` = apophis + carter; apophis rebuilt on ZFS; `pvesr` replication + **manual failover** for VM 200; 2nd Technitium on carter for DNS redundancy; corosync ride-out; cluster-aware monitoring). **Phase 5 next** (Secrets + HA expansion — Vaultwarden; swapped ahead of Media 2026-06-25). Terraform import deferred to cluster scale (ADR-008). Order: 1 VLANs ✓ → 2 Tailscale + Technitium ✓ → 3 Foundation + observability ✓ → 4 Multi-node cluster + HA ✓ (apophis + carter; oneill stays **standalone**; ZFS replication; **no HA manager/fencing** — manual failover) → 5 Vaultwarden + HA expansion → 6 Jellyfin + media. Cross-cutting: backups + patching.
