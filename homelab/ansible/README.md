...
| `provision-actual.yml` | Actual Budget VM 127 on Carter — pinned official container, loopback-only app port, Tailscale Serve HTTPS, persistent data, and encrypted PBS enrolment (ADR-023). Optionally builds the manual category-only monthly AI overlay at `/insights` after its four controller secret files are staged (ADR-024); disabled by default. Reads a one-use tagged key from protected `actual_ts_authkey_file` for the initial join; reruns do not require the file |
