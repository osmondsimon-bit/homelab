...
| `provision-actual.yml` | Actual Budget VM 127 on Carter — pinned official container, loopback-only app port, Tailscale Serve HTTPS, persistent data, and encrypted PBS enrolment (ADR-023). Reads a one-use tagged key from protected `actual_ts_authkey_file` for the initial join; reruns do not require the file |
