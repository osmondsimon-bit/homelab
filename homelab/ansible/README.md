...
| `deploy-monitoring-rules.yml` | Stages and `promtool`-validates the committed Prometheus rules in CT 114, promotes them atomically, and reloads only Prometheus. Use for rule-only changes; it does not recreate PVE API tokens, prompt for secrets, or restart the monitoring stack |
