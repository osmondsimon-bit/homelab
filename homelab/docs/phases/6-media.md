...
- **USB mount monitoring** uses a dedicated five-minute node_exporter textfile collector. Missing
  mount is critical, absent/stale telemetry and 85% capacity are warnings, and Glance never falls
  through to reporting the host root filesystem as media capacity.
...
- Media storage metrics: `homelab_media_storage_mounted`, `*_used_bytes`, `*_available_bytes`, and
  `*_size_bytes` from `provision-media-storage-monitoring.yml`.
