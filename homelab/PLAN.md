...
3. **Foundation + observability:** PBS/mgmt-vm backups ✓ → **Monitoring** ✓ (Prometheus + Grafana + Alertmanager) → **Glance** ✓ (front-door dashboard, ADR-014; was Homepage). **✓ closed via `/phase-gate` 2026-06-17** — see `docs/phases/3-foundation-observability.md`. Carry-forwards (clear before new stateful services in Phase 4): ~~HA native backup~~ ✅ landing confirmed, ~~first restore drill~~ ✅ PASS (PBS/mgmt-vm) — both 2026-06-17; **remaining: PBS encryption decision + an HA-native-restore drill** (the latter gates retiring the held `vzdump-qemu-200`). (Terraform import deferred to cluster scale — ADR-008.)
...
- **Phase 3 ✓ CLOSED** (`/phase-gate` 2026-06-17 — doc-auditor + continuity-reviewer + security-review, record in `docs/phases/3-foundation-observability.md`). **Next: Phase 4** (cluster + HA). Phase 3 backup carry-forwards: mgmt-vm restore drill ✅ PASS + HA native backup ✅ landing (both 2026-06-17); **remaining before new stateful services: PBS encryption decision + an HA-native-restore drill.**
...
- [~] **[High] HA backup encryption key — SAVE IT OFF-BOX.** The HAOS backups are **encrypted** (`"protected": true`). The key (HAOS → Settings → System → Backups → ⋮ → "Show encryption key") must be stored where it survives losing oneill/HA. **No credential manager is set up yet** (ADR-010's Bitwarden bridge is not live) — saving it in **Google Password Manager** for now. Without the key, the encrypted backup is unrestorable.
...
- [~] **[High] PBS encryption — confirmed NOT enabled (2026-06-17).** `/etc/pve/priv/storage/` has only `pbs-oneill.pw` (datastore login), no `pbs-oneill.enc` (client-side key) → backups are unencrypted at rest on oneill. ADR-012 wants it on. **Decision pending:** enable (`pvesm set pbs-oneill --encryption-key autogen`) — then the generated key at `/etc/pve/priv/storage/pbs-oneill.enc` **must** be saved off-box (Google Password Manager, like the HA key) or future encrypted backups are unrestorable; existing backups stay unencrypted.
