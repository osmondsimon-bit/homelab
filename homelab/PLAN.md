...
- [x] **Phase 4 first steps — defined 2026-06-22** (infra-designer reviewed). `carter` = ThinkCentre-class SFF, **i5-8500 / 32 GB / 256 GB (verify online — check disk is SSD)** — 8th-gen Coffee Lake, **same CPU gen as apophis** → migration-compatible pair. Ordered cluster-formation sequence (ADR-009):
...
- [ ] **Verify carter specs once online** — expected i5-8500 / 32 GB / 256 GB. Confirm CPU + RAM, and **check the disk is an SSD — if it's a spinning HDD, swap it** (ZFS + running the HA VM on failover off a mechanical disk = poor). 256 GB caps what can replicate to it (the HA-set only, which is correct).
...
- [~] **Autostart audit (`onboot` + startup order/delay)** — audited 2026-06-22: **`onboot=1` is already set on ALL guests** (apophis VM 100/200 + CT 110; oneill CT 111–116) → guests auto-recover after a reboot. **Remaining:** no `startup` order/delay set anywhere — add ordering (Technitium=1 → deps=2 → HA VM=3) + a small stagger so boot isn't a thundering herd and DNS is up first. Procedure in `docs/operations/runbooks.md` (power-loss/autostart section).
- [ ] **Power-loss behaviour per host (BIOS AC recovery)** — confirm each host (apophis, oneill, carter) is set to **power on after AC loss** in BIOS ("After Power Loss" = Power On, not Stay Off / Last State). **Unknown whether oneill auto-returns today — verify.** Plus confirm the **UPS also covers the network device** so a power blip doesn't drop the whole LAN (the common-mode failure). Per-host steps + a safe verification test in the runbook.
