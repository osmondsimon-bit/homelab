...
3. **Foundation + observability:** PBS/mgmt-vm backups ✓ → **Monitoring** ✓ (Prometheus + Grafana + Alertmanager) → **Glance** ✓ (front-door dashboard, ADR-014; was Homepage). **✓ closed via `/phase-gate` 2026-06-17** — see `docs/phases/3-foundation-observability.md`. Carry-forwards (clear before new stateful services in Phase 4): ~~HA native backup~~ (✅ landing confirmed 2026-06-17), PBS encryption check, first restore drill (which also verifies the HA partial backup + retires its held vzdump). (Terraform import deferred to cluster scale — ADR-008.)
...
- **Phase 3 ✓ CLOSED** (`/phase-gate` 2026-06-17 — doc-auditor + continuity-reviewer + security-review, record in `docs/phases/3-foundation-observability.md`). **Next: Phase 4** (cluster + HA) — **but FIRST clear the remaining Phase 3 backup carry-forwards below** (PBS encryption, first restore drill; HA native backup ✅ landing confirmed 2026-06-17) before any new stateful service lands.
