...
- [x] **Phase 4 first steps — defined 2026-06-22** (infra-designer reviewed). `carter` = ThinkCentre-class SFF, **i5-8500 / 32 GB / 256 GB SSD** (confirm CPU/RAM online) — 8th-gen Coffee Lake, **same CPU gen as apophis** → migration-compatible pair. Ordered cluster-formation sequence (ADR-009):
  1. **Pre-work (before any cluster command):** answer the **UDM DNS-fallback question** (does home-VLAN DNS survive Technitium being down? — see line above); **reserve carter's static IP in UniFi** (avoid the Glance .12 collision lesson); pick an oneill-evacuation window that misses the HAOS scheduled backup.
  2. **Install carter:** Proxmox on **ZFS RAID-0** (pool `rpool` / storage `local-zfs`, matching oneill), **PVE version matched to oneill (9.x)**, static IP, hostname `carter`, **no guests** (a node can only *join* a cluster if it has none).
  3. **Create the cluster ON apophis** (originator keeps its guests — HA VM 200 etc. — with zero disruption).
  4. **Join carter** (empty → trivial). ⚠️ **Two-node quorum window** (apophis+carter) starts here: cluster loses quorum if *either* reboots — keep this window short, no planned reboots until oneill joins.
  5. **HARD GATE — stand up 2nd Technitium on carter** and register **both** resolvers in UniFi DHCP **before** destroying oneill's CT 111. This is a prerequisite, not "later" — otherwise home-VLAN DNS dies for the whole evacuation.
  6. **Evacuate oneill** (CTs reproducible from Ansible): destroy CTs 111–116, verifying each service healthy on its new home first; **PBS (CT 112) destroyed last** to keep backing up as long as possible.
  7. **Join oneill** (now empty), then **re-provision its CTs via Ansible**; re-register the PBS datastore as cluster storage; add carter to `monitoring_node_targets` + `monitoring_pve_nodes` and re-run the monitoring playbook (ADR-017 onboarding for carter).
  8. **Migrate Tailscale CT 110** apophis → oneill (completes ADR-009 service placement).
  9. **Then 4b:** apophis **LVM-thin → ZFS** migration (backup→rebuild→restore via PBS, with carter/oneill hosting HA VM during the rebuild) → set up **scheduled ZFS replication** (apophis↔carter, VMID 200) + a **manual-failover runbook**. **No automatic HA manager** (ADR-009 revised 2026-06-22 — unsafe on a single-network-path lab; would fence healthy nodes on a network blip). Replication needs apophis on ZFS first.

  Natural scope split (resolves the item below): **4a = steps 1–8** (cluster + DNS redundancy + service placement); **4b = step 9** (apophis ZFS migration + replication + manual failover + Terraform import).
