...
### Decisions to make
- [~] **Patching/update approach — ADR-015 ACCEPTED (2026-06-17); not yet implemented.** Decided: `unattended-upgrades` **security-only, no auto-reboot**, timer at **midday**, on the Debian **service LXCs**; **mgmt-vm + Proxmox hosts patched manually in a monthly window — last day of month, 12:00 AEST** (be present for fallout, not overnight); HAOS via its own UI; host-prep play codifies the `pve-no-subscription` switch. **Next:** build `provision-patching.yml` (gated: infra-designer + `/security-review`). Patching is manual/ad-hoc until then — oneill already shows a real pending-update backlog. Visibility (hosts) is live via Glance (`apt_upgrades_pending`, `node_reboot_required`).
