...
- [ ] **UniFi read access for the agent (reviews + troubleshooting).** Today the agent only sees UniFi via unpoller *metrics*, not *config* (VLANs, firewall rules, port profiles, clients). **Plan:** create a **read-only UniFi API key**, store it in gitignored local config (mirror `~/.technitium-ro-token`), add `scripts/unifi-query.sh` helper. Read-only, LAN-only. A read-only **UniFi MCP** is the nicer interactive path but stays parked until MCP is verified in this env — API key first. Synergy: enables the agent to read the current topology to help design the new-house network below.
