...
### Decisions to make
- [~] **Patching/update approach — ADR-015 DRAFTED (Status: Draft); not implemented yet.** Shape: `unattended-upgrades` **security-only, no auto-reboot** on the Debian/Ubuntu guests; a deliberate **monthly rolling window** on the Proxmox hosts (zero-downtime once Phase 4 HA failover exists); HAOS updated via its own UI; host-prep play codifies the `pve-no-subscription` repo switch. **Next:** confirm the open choices in ADR-015 (window time; whether mgmt-vm auto-patches), then build `provision-patching.yml` (gated: infra-designer + `/security-review`). Patching is manual/ad-hoc until then — oneill already shows a real pending-update backlog. Visibility (hosts) is live via Glance (`apt_upgrades_pending`, `node_reboot_required`).
