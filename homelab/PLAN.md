...
- [x] **Phase 4 first steps — defined 2026-06-22** (infra-designer reviewed). `carter` = 2nd ThinkCentre M920Q (i7-8700T, **same CPU gen as apophis** → clean cold-restart HA failover between them). Ordered cluster-formation sequence (ADR-009):
  ...
  9. **Then 4b:** apophis **LVM-thin → ZFS** migration (backup→rebuild→restore via PBS, with carter/oneill hosting HA VM during the rebuild) → configure **ZFS replication** (apophis↔carter, VMID 200) → enable **HA manager**. True HA failover cannot start until apophis is on ZFS.

  Natural scope split (resolves the item below): **4a = steps 1–8** (cluster + DNS redundancy + service placement); **4b = step 9** (apophis ZFS migration + replication + HA failover + Terraform import).
...
- [x] **Phase 4 scope split — decided 2026-06-22:** **4a** = cluster formation + carter + 2nd Technitium/DNS redundancy + oneill rejoin + service placement (Tailscale→oneill); **4b** = apophis LVM→ZFS migration + ZFS replication + HA failover for VMID 200 + Terraform import. See the step-by-step under "Phase 4 readiness" above.
...
- [ ] **HA group + scope** — create HA group `core-ha` = {apophis, carter}; HA-flag **VMID 200 only** (Vaultwarden later); set those VMs to a **fixed CPU type** (e.g. `x86-64-v2-AES`), not `host`, so live-migration across the pair is guaranteed. oneill excluded from the group.
- [ ] **Corosync ride-out** — set `totem { token: 10000 }` in `/etc/pve/corosync.conf` (bump `config_version`) so short switch/gateway blips don't drop membership. (Won't cover multi-minute reboots — nothing can on single-NIC nodes; that's the accepted SPOF per ADR-009.)
- [ ] **Manual UniFi control-plane updates** — disable UniFi **auto-update** on the gateway + switch; write a `docs/operations/runbooks.md` runbook: *put cluster in HA maintenance → update the device → resume*. Stops a UniFi auto-update reboot from fencing the cluster (a UniFi gateway auto-update reboot was the root cause of the 2026-06-21 Zigbee outage — see the Zigbee alerting rationale in `ansible/files/monitoring/alert-rules.yml`).
...
- [ ] **(Future option) 2nd corosync ring** — USB-Gigabit NIC per host on a small separate switch = true redundancy against the shared device dying. Deferred (USB-NIC jitter; secondary-ring only) — revisit with the new-house network.
