...
- [~] **Power-loss behaviour per host (AC recovery)** — 2026-06-22: **apophis set to "Power On" REMOTELY** via `thinklmi` (no console, no BIOS password). **carter:** same one-liner once on the network (Lenovo). **oneill (NUC): TESTED — does NOT auto-recover** (AC pulled while running → stayed off → needed a manual power-button press). No remote interface, so it **requires a physical F2 BIOS visit: Power → Secondary Power Settings → "After Power Failure" = Power On.** Until then a power blip = oneill stays down (DNS/monitoring/PBS offline until someone presses the button). **Still TODO:** confirm the **UPS also covers the network device**. Method in `docs/operations/runbooks.md` (§A).
