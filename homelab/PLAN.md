...
- [~] **Phase 4 sequence — REVISED 2026-06-22 to a 2-node cluster (apophis + carter); oneill stays standalone** (see ADR-009). Rationale: we're doing **manual failover, not auto-HA**, so the only thing 3-node membership would add is seamless quorum — not worth evacuating oneill (which would drop DNS) when its guests are reproducible from Ansible. `carter` = i5-8500 / 32 GB / 256 GB SSD, 8th-gen Coffee Lake (matched to apophis → migration-compatible pair).
  ...
  4. **4b — rebuild apophis on ZFS** (apophis is the only LVM-thin node; needs a reinstall). **Live-migrate** HA VM 200 + mgmt-vm 100 apophis→carter (`--targetstorage local-zfs`; HA stays up on carter — mgmt-vm migration you drive from the UI since Claude runs on it); rebuild Tailscale 110 on **oneill** (reproducible, standalone — no cluster needed).
  5. **`pvecm delnode apophis`** → reinstall apophis with **ZFS-on-root** (`rpool`/`local-zfs`, same version) → **rejoin** cluster, re-authorise SSH key.
  6. **Migrate 100/200 back to apophis**, set up **`pvesr` ZFS replication apophis↔carter for VM 200** + the manual-failover runbook. (Plex/media on apophis's iGPU is Phase 5 — not a factor in the rebuild.)
