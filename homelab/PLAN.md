...
- [x] **[High] HA native backup — landing confirmed (2026-06-17).** Samba CT 113 (`provision-ha-backup-share.yml`) + HAOS scheduled **PARTIAL** backup; automatic backups are recurring onto the share (~131 MB each). Remaining: confirm in HAOS the partial scope includes the **Zigbee2MQTT add-on** and **excludes media**, and tune recorder `purge_keep_days`.
