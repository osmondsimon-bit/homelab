...
- [~] **Autostart (`onboot` + startup order/delay)** — 2026-06-22: `onboot=1` already on all guests, and **`startup` ordering now applied live** (oneill: Technitium=1 → PBS/HA-share=2 → Monitoring=3 → Glance/portal=4; apophis: Tailscale=1 → HA VM=2 → mgmt-vm=3, with `up=` stagger). **Remaining:** codify `--startup` in the CT provisioning playbooks (alongside `--onboot`) so a rebuild preserves it — fold into the Phase 4 oneill reprovision (CTs 111–116 get recreated then anyway). Procedure in `docs/operations/runbooks.md`.
- [~] **Power-loss behaviour per host (AC recovery)** — 2026-06-22: **apophis set to "Power On" REMOTELY** via the `thinklmi` sysfs interface (no console; no BIOS password) — Lenovo ThinkCentres don't need a physical BIOS visit. **carter:** same one-liner once on the network. **oneill (NUC):** no remote interface — behavioural power-toggle test, or one physical F2 visit (likely defaults to "Last State", which returns an always-on box). **Still TODO:** confirm the **UPS also covers the network device**. Method in `docs/operations/runbooks.md` (power-loss/autostart §A).
