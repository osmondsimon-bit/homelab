...
- [x] **UniFi read access for the agent — done 2026-06-19.** `scripts/unifi-query.sh` (GET-only, cookie auth, cached session) reuses the read-only `prometheus_user` (unpoller) account; creds in gitignored `~/.unifi-ro.env` (chmod 600). Verified: networkconf/wlanconf/firewallgroup/user/device/sta/health all `rc=ok`. First review captured the topology (VLANs Home 2 / Camera 3 / IoT 4 / Guest 99 / Mgmt 254 — matches docs). **Follow-ups:** (a) firewall is **zone-based** → classic `rest/firewallrule` is empty; add **v2 API** (`/proxy/network/v2/api/site/<site>/firewall-policies`) to the helper to review policies; (b) cleanup candidates surfaced — leftover **WireGuard Client** network (ADR-003 superseded WG w/ Tailscale; vpn health `unknown`) and a vestigial untagged **Default** 192.168.1.x net (0 clients). MCP path still parked. Now enables reading current topology to design the new-house network.
