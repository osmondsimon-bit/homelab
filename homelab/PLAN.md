...
- [ ] **mgmt-vm 10GB RAM reboot** — config set 2026-06-20 (`qm set 100 --memory 10240`); **reboot PENDING** (drops session — do at session end).
...
**Phase 4 readiness (node `carter` arrived 2026-06-22):**
- [~] **Phase 4 sequence — REVISED 2026-06-22 to a 2-node cluster (apophis + carter); oneill stays standalone** (see ADR-009). Rationale: we're doing **manual failover, not auto-HA**, so the only thing 3-node membership would add is seamless quorum — not worth evacuating oneill (which would drop DNS) when its guests are reproducible from Ansible. `carter` = i5-8500 / 32 GB / 256 GB SSD, 8th-gen Coffee Lake (matched to apophis → migration-compatible pair).
  1. **Onboard carter — ✅ DONE 2026-06-22:** SSH key authorised, no-subscription repos set, updated to pve-manager **9.2.3** (matched) + rebooted, `rpool`/`local-zfs` confirmed, added to inventory. No guests.
  2. **Create cluster on apophis — ✅ DONE 2026-06-22** (`pvecm create homelab`; apophis guests kept running, zero disruption). No HA-flagged VMs (no fencing).
  3. **Join carter — ✅ DONE 2026-06-22.** Gotchas hit & recorded: (a) apophis root has **2FA**, so the **GUI/API join fails** ("TFA-enabled login currently works only with a TTY") — do `pvecm add YOUR_PROXMOX_IP` (apophis) from **carter's shell (a TTY)** instead, entering the apophis password + 2FA code; (b) joining **wipes the joiner's `storage.cfg`**, so `local-zfs` was re-added (`pvesm add zfspool local-zfs --pool rpool/data --content images,rootdir --nodes carter`) and `local-lvm` scoped to apophis. Cluster `homelab` is **2-node, Quorate**. **2-node quorum:** a node-down → `/etc/pve` read-only (VMs keep running); manual failover runs **`pvecm expected 1`** first (runbook). oneill **not** involved.
  4. **4b — rebuild apophis on ZFS** (the only LVM-thin node; needs a reinstall). **Full step-by-step + VERIFY gates + rollback + the manual-failover runbook are in `docs/operations/runbooks.md` → "Phase 4b: rebuild apophis on ZFS" (infra-designer-reviewed 2026-06-22).** Shape: evacuate VM 200 + 100 to carter → reinstall apophis ZFS-on-root → rejoin → migrate back → `pvesr` replication for VM 200. HA stays up on carter throughout. Plex/iGPU is Phase 5 (unaffected). **Critical pre-flight from the review — do NOT start the destructive step until these pass:**
     - **CPU type — decided `x86-64-v2-AES` (for clean live migration).** Both VM configs set 2026-06-22. **VM 200 (HA) ✅ applied** (restarted; running `-cpu qemu64,+aes,+sse4.2,…` = v2-AES; HA serving). **VM 100 (mgmt-vm) — restart PENDING** (config set, still running `host`; can't self-restart — Simon restarts it from the Proxmox UI to apply, before its 4b live-migration; the session drops+reconnects).
     - **carter AC power-recovery — ✅ DONE 2026-06-22:** set "After Power Loss" = Power On remotely via `thinklmi` (was "Last State", no BIOS password). carter (sole production host during the apophis reinstall) now self-boots after a power loss.
     - **verify the PBS encryption key off-box** (fingerprint matches password-manager copy) before the reinstall wipes apophis.
     - **`pvecm expected 1` before `pvecm delnode apophis`** (apophis powered off first), else carter goes read-only.
     - rejoin is via `pvecm add` from **apophis's TTY shell** (carter root has cluster-wide 2FA → GUI/API join fails, like the first time). **`/etc/pve` is cluster-shared**, so users/2FA/ACLs/monitoring-token/PBS-key/storage.cfg/VM-configs all return automatically on rejoin — only node-local items need redoing: reset apophis SSH host key (`ssh-keygen -R` + re-`ssh-copy-id` the mgmt-vm key), no-sub repos, and `node_exporter` (`install-node-exporter.yml --limit apophis`).

  **oneill untouched throughout** → no DNS/monitoring/PBS disruption. **carter monitoring onboarding (ADR-017) — ✅ DONE 2026-06-22:** node_exporter installed; added to `monitoring_node_targets` + `monitoring_pve_nodes` (with `token_from: apophis` — clustered nodes share one cluster-wide PVE token, so the playbook now mints once and carter reuses it; fixed a playbook race that had briefly broken apophis/oneill pve-exporter auth); added to `glance_hosts` (host pane live). All node + pve-exporter targets up. Independent (non-gating) backlog: DNS secondary (gateway `.2.1` in UniFi DHCP) and/or a 2nd Technitium for redundancy. **Follow-up (minor):** clustered pve-exporters (apophis+carter) now report **duplicate cluster-wide `pve_*` series** — kept deliberately for resilience (cluster metrics survive one node down), but dedupe in alert rules / Glance widgets with `max by (id) (...)` if double-counting/alert-noise shows up. Scope: **4a = steps 2–3** (cluster), **4b = steps 4–6** (apophis ZFS + replication; Terraform import decision deferred — see below).
- [x] **DNS redundancy** — decided: **after clustering** (Phase 4, once new node joins). Second Technitium instance as Phase 4 step once cluster is formed.
- [x] **Phase 4 scope split — decided 2026-06-22:** **4a** = cluster formation + carter + 2nd Technitium/DNS redundancy + oneill rejoin + service placement (Tailscale→oneill); **4b** = apophis LVM→ZFS migration + ZFS replication + **manual failover** for VMID 200 + Terraform import. See the step-by-step under "Phase 4 readiness" above.

**Phase 4 cluster-hardening backlog (captured 2026-06-22 — execute during/after cluster formation, ADR-009 refinements):**
- [ ] **Verify carter specs once online** — expected i5-8500 / 32 GB / **256 GB SSD (confirmed SSD)**. Confirm CPU + RAM online. 256 GB caps what can replicate to it (the HA-set only, which is correct).
- [ ] **Replication + MANUAL failover (NO HA manager — ADR-009 revised 2026-06-22)** — schedule ZFS replication of **VMID 200** (Vaultwarden later) apophis↔carter every 5–15 min; **do not HA-flag any VM** (keeps the watchdog disarmed → a network blip never fences a node). Set those VMs to a **fixed CPU type** (e.g. `x86-64-v2-AES`), not `host`, so live-migration across the pair is guaranteed. Write the manual-failover runbook (start the VM on the survivor from the latest replicated snapshot).
- [~] **Autostart (`onboot` + startup order/delay)** — 2026-06-22: `onboot=1` already on all guests, and **`startup` ordering now applied live** (oneill: Technitium=1 → PBS/HA-share=2 → Monitoring=3 → Glance/portal=4; apophis: Tailscale=1 → HA VM=2 → mgmt-vm=3, with `up=` stagger) — **validated live by oneill's 2026-06-22 power-cycle: all 6 CTs returned automatically.** **Remaining:** codify `--startup` in the CT provisioning playbooks (alongside `--onboot`) so a rebuild preserves it — fold into the Phase 4 oneill reprovision (CTs 111–116 get recreated then anyway). Procedure in `docs/operations/runbooks.md`.
- [~] **Power-loss behaviour per host (AC recovery)** — **apophis ✅** "Power On" set remotely via `thinklmi` (2026-06-22). **oneill ✅ FIXED 2026-06-22** — generic AMI Aptio BIOS (no remote interface); root cause was **"State After G3" = S5** (stay off), changed to **S0** (power on) — confirmed self-boots on AC restore. (On this board the setting is "State After G3", not "Restore AC Power Loss".) **carter ✅ DONE 2026-06-22** — "After Power Loss" = Power On set remotely via `thinklmi` (was "Last State"). **All three hosts now auto-recover from power loss.** **Still TODO:** confirm the **UPS also covers the network device**. Method in `docs/operations/runbooks.md` (§A).
- [ ] **Corosync ride-out (comfort, not safety)** — set `totem { token: 10000 }` in `/etc/pve/corosync.conf` (bump `config_version`) so short blips don't needlessly drop quorum (→ brief `/etc/pve` read-only). With no HA flags a blip can't fence anyway — this just reduces nuisance. Won't cover multi-minute reboots (the accepted SPOF, ADR-009).
