...
- **[Infra review] Re-balance RAM now apophis has 32 GB** (upgraded 2026-06-18; host now ~25 GB free, was tight at 16 GB). Snapshot 2026-06-18 (used / alloc / 24h-peak% — from Prometheus, Grafana still empty):
  - **home-assistant** 2.2 / 4.3 G — **24h peak 92%** → bump to **6 GB** (memory-pressured).
  - **mgmt-vm** 1.7 / 4.3 G — **24h peak 86%** → bump to **6 GB**.
  - tailscale 256 MB — peak 66% → optional bump to 512 MB. technitium / pbs / monitoring / glance / ha-share all <30% peak — leave as is.
  - **How:** HA (VM 200) + mgmt-vm (VM 100) are hand-built VMs (not Ansible) → `qm set 200 --memory 6144` / `qm set 100 --memory 6144` + reboot the guest (no hotplug configured). Re-check 24h peaks after a few days. Tailscale RAM is in group_vars (`tailscale_ram_mb`) → playbook + reprovision.
